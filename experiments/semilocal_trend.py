...
class SemiLocalTrend(distribution.Continuous):
    ...
    Describes distributions of sequences del
    to the equation:
...
class AR1(distribution.Continuous):
    ...
    def logp(self, x):
        ...
        # Log probability of the subsequent sequenlce
...
class CauchyAR1(distribution.Continuous):
    """An Autoregressive distribution order with Cauchy Noise.

    A distribution of sequences of AR1 processes specified by the equations.

        theta_t+t = phi*theta_t + epsilon_t
        theta_0 = epsilon_0
        epsilon_t ~ cauchy(0, eps_sd)

    Attributes:
        phi (float): The persistence term.
        scale (float): Scale parameter of the cauchy error.
    """
    def __init__(self, phi, scale, *args, **kwargs):
        super(CauchyAR1, self).__init__(*args, **kwargs)
        self.phi = tt.as_tensor_variable(phi)
    ...
    def logp(self, x):
        ...
        # Log probability of the first observation
        logp_initial = pm.Cauchy.dist(alpha=0.0, beta=self.scale).logp(x_init)

        # Log probability of the subsequent sequence
        location = self.phi * x_prev
        logp_seq = pm.Normal.dist(alpha=location, beta=self.scale).logp(x_now)

        logp_total = logp_initial + tt.sum(logp_seq)
    ...
    def random(self, length, size):
        ...
        """
        shape = list(size) + [length]
        x_base = pm.Cauchy.dist(alpha=0.0, beta=self.scale).random(size=shape)
        # Type casting necessary since we accumulate an autogenerated ufunc
        result = self.disc_add.accumulate(
            x_base, axis=-1, dtype=np.object).astype(np.float)
        return result
